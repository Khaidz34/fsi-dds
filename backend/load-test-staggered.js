...
/**
 * Load Test Script - Staggered requests to avoid rate limiting
 * Usage: node backend/load-test-staggered.js
 */
...
// Register test users with staggering
const registerUsers = async () => {
  console.log(`\n📝 Registering ${NUM_USERS} test users (staggered)...`);
...
// Place orders with staggering
const placeOrders = async () => {
  console.log(`\n🍽️ Placing ${NUM_USERS * ORDERS_PER_USER} orders (staggered)...`);
    ...
    for (let i = 0; i < ORDERS_PER_USER; i++) {
      ...
      try {
        const response = await makeRequest('POST', '/api/orders', {
          ...
          dish2Id: dish2.id,
          orderedFor: user.id,
          notes: `Test order ${i + 1} from ${user.username}`
        }, user.token);
        ...
        if (response.status === 201 || response.status === 200) {
          ...
          console.log(`✓ Order ${orderCount} placed by ${user.username} (${responseTime}ms)`);
        } else {
          metrics.failedRequests++;
          metrics.errors.push(`${user.username}: ${response.status}`);
          console.log(`✗ Order ${orderCount} failed for ${user.username} (${response.status})`);
        }
      } catch (err) {
        metrics.failedRequests++;
        metrics.errors.push(`${user.username}: ${err.message}`);
        console.log(`✗ Error placing order ${orderCount} for ${user.username}: ${err.message}`);
      }

      // Stagger delay between orders
      await sleep(STAGGER_DELAY);
    }
  }
...
const displayMetrics = () => {
  ...
  if (metrics.errors.length > 0) {
    console.log(`\n❌ Errors:`);
    const errorSample = metrics.errors.slice(0, 10);
  ...
  if (successRate >= 95) {
    console.log('✅ EXCELLENT: System handles 30 concurrent users well');
  } else if (successRate >= 80) {
    console.log('⚠️ GOOD: System mostly handles load but needs minor optimization');
  } else if (successRate >= 50) {
    console.log('⚠️ FAIR: System partially handles load but needs optimization');
  } else {
