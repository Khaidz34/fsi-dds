-- PostgreSQL function to calculate payment stats - ONLY USERS WITH DEBT (FIXED)
-- Returns only users who have remainingTotal > 0
-- Uses orders.paid boolean instead of payments table
-- FIXED: Only count orders where user is the RECEIVER (ordered_for), not the orderer
